from flask import Flask
from flask import request,jsonify


thelibs = [

]


# Lexer and Parser of the Oxygen Programming Language
# Desinged for production use for runner
# (c) Arjun J

import sys
import json
import os
import requests

class Oxygen:
    def __init__(self, code):
        self.code = code
    def lexer(self):
        code = self.code
        length = len(code)
        pos = 0
        tokens = [{'type': 'keyword', 'value': 'nothing'}]
        custom_keywords = []
        BUILT_IN_KEYWORDS = ["print", "function", "import", "nothing"]
        chars = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_()""\',=-!@#$^&*()\{\}|:"<>?,./~`;[]\\'
        mathoperators = '+-/*%'
        allowedfunc = []
        allowedkey = []
        while pos < length:
            currentchar = code[pos]
            if currentchar == " " or currentchar == '\n':
                pos += 1
                pass
            elif currentchar == '"':
                res = ""
                pos += 1
                while code[pos] != '"' and code[pos] != '\n' and pos < length:
                    res += code[pos]
                    pos += 1
                if code[pos] != '"':
                    return {
                        "error": "String does not end with \""
                    }
                pos += 1

                tokens.append({
                    "type": "string",
                    "value": res
                })
            elif currentchar in chars:
                res = currentchar
                pos += 1


                chars1 = "1234567890"
                if code[pos - 1] == ",":
                    chars1 += " "

                while code[pos] in chars + chars1 and code[pos] != "\n" and code[pos] != "\r" and pos < length:
                    res += code[pos]
                    pos += 1

                if res not in BUILT_IN_KEYWORDS:
                    if tokens[len(tokens) - 1]["value"] != "function":
                        if res not in allowedfunc:
                            works = False
                            for x in allowedkey:
                                if res.split("(")[0] + "(" in x:
                                    works = True
                            if res == ")":
                                works = True

                            if works == False:
                                return {
                                    "error": 'Unexpected token "%s"' % res
                                }
                            else:

                                tokens.append({
                                    "type": "run",
                                    "value": res
                                })



                        else:
                            if res in allowedfunc and res in allowedkey:
                                return "Function and module both have the same value"
                            elif res in allowedfunc:
                                tokens.append({
                                    "type": "functioncalled",
                                    "value": res
                                })
                            else:
                                for x in allowedkey:
                                    if res.split("(")[0] + "(" in x:
                                        tokens.append({
                                            "type": "run",
                                            "value": res
                                        })

                    else:
                        tokens.append({
                            "type": "funcname",
                            "value": res
                        })

                        allowedfunc.append(res)

                        try:
                            if code[pos + 1] == " ":
                                pos += 1
                        except:
                            pass

                        pos += 1

                        res = ""

                        try:
                            while code[pos] != ":" and pos < length:
                                res += code[pos]
                                pos += 1
                        except:
                            return {
                                "error": "Expected \":\""
                            }


                        pos += 1

                        tokens.append({
                            "type": "funcdef",
                            "value": res
                        })
                else:
                    tokens.append({
                        "type": "keyword",
                        "value": res
                    })

                    if res == "import":
                        try:

                            res = ""
                            if code[pos + 1] == " ":
                                pos += 1
                            pos += 2
                            while code[pos] != '"' and code[pos] != '\n' and pos < length:
                                res += code[pos]
                                pos += 1
                            checkmodulecommands = open("opl/" + res + "/commands.json")
                            x = checkmodulecommands.read()
                            y = json.loads(x)
                            for z in y:
                                allowedkey.append(z)
                            tokens.append({
                                "type": "importdef",
                                "value": res
                            })
                            pos +=1
                        except Exception as e:
                            return {
                                "error": "Could not import module: " + str(e)
                            }

            elif currentchar in mathoperators:
                tokens.append({
                    "type": "op",
                    "value": currentchar
                })
                pos+=1
            elif currentchar in "1234567890":
                res = currentchar

                pos += 1
                while code[pos] in "1234567890." and pos < length:
                    res += code[pos]
                    pos += 1

                tokens.append({
                    "type": "num",
                    "value": float(res)
                })
            else:
                return {
                    "error": 'Unexpected character "%s"' % code[pos]
                }
        return {
            "error": False,
            "tokens": tokens
        }

    def parse(self, tokens):

        customfunc = []
        functionvalue = []
        commands = []
        length = len(tokens)
        pos = 0
        while pos < length:
            token = tokens[pos]

            if token['type'] == "keyword" and token['value'] == "print":
                try:
                    if tokens[pos + 1]:
                        pass
                except:
                    return "Unexpected end of line, expected string"
                    sys.exit()

                isString = tokens[pos + 1]['type'] == "string"
                if (isString == False):
                    return "Unexpected token \"" + tokens[pos + 1]['value'] + "\", expected string"
                    sys.exit()

                return tokens[pos + 1]['value']
                pos += 2
            elif token["type"] == "op":
                try:
                    if tokens[pos - 1]["type"] != "string" and tokens[pos - 1]["type"] != "num":
                        return "Expected type string or num, not " + tokens[pos - 1]["type"]
                        sys.exit()
                    else:
                        if tokens[pos + 1]["type"] != tokens[pos - 1]["type"]:
                            return "Expected type " + tokens[pos - 1]["type"] + ", not " + tokens[pos + 1]["type"]
                            sys.exit()
                        else:
                            if tokens[pos + 1]["type"] == "string" and token["value"] != "+":
                                return "Cannot perform operation, string can only perform \"+\""
                                sys.exit()
                            else:
                                if tokens[pos + 1]["type"] == "string":
                                    res = tokens[pos - 1]["value"] + tokens[pos + 1]["value"]
                                    return res
                                else:
                                    operation = token["value"]
                                    if operation == "+":
                                        return tokens[pos - 1]["value"] + tokens[pos + 1]["value"]
                                    if operation == "-":
                                        return tokens[pos - 1]["value"] - tokens[pos + 1]["value"]
                                    if operation == "*":
                                        return tokens[pos - 1]["value"] * tokens[pos + 1]["value"]
                                    if operation == "/":
                                        return tokens[pos - 1]["value"] / tokens[pos + 1]["value"]
                                    if operation == "%":
                                        return tokens[pos - 1]["value"] % tokens[pos + 1]["value"]
                                pos += 2
                except:
                    return "Can not find one of the one of the arguments required for performing the operation"
                    sys.exit()
            elif token["type"] == "keyword" and token["value"] == "function":
                if tokens[pos + 1]["type"] != "funcname":
                    return "Expected a function name, but instead got a " + tokens[pos + 1]["type"]
                    sys.exit()
                customfunc.append(tokens[pos + 1]["value"])
                pos += 2
            elif token["type"] == "funcdef":
                functionvalue.append(token["value"])
                pos += 1
            elif token["value"] in customfunc and token["type"] == "functioncalled":
                self.code = "\n" + functionvalue[customfunc.index(token["value"])] + "\n"
                self.parse(self.lexer()["tokens"])
                pos += 1
            elif token["type"] == "keyword" and token["value"] == "import":
                if tokens[pos + 1]["type"] != "importdef":
                    return "Expected type importdef, but instead got " + tokens[pos + 1]["type"]
                    sys.exit()
                pos += 1
            elif token["type"] == "importdef":
                exec("from opl." + token["value"] + ".run import *", globals(), locals())
                pos += 1
            elif token["type"] == "run":
                exec(token["value"], globals(), locals())
                pos += 1
            elif token["type"] == "string":
                pass
                pos += 1
            elif token["type"] == "num":
                try:
                    if tokens[pos + 1]["type"] == "op":
                        pass
                    else:
                        return token["value"]
                except:
                    return token["value"]
                pos += 1
            elif token["type"] == "keyword" and token["value"] == "nothing":
                pass
                pos += 1
            else:
                return "Unexpected token " + token["type"]
                sys.exit()


app = Flask(__name__)

@app.route('/')
def hello_world():
    return 'The is the runner for Oxygen, a programming language. You might of went here by mistake. This is used for deployment.'

@app.route('/run/')
def runhome():
    return "The runner"

@app.route('/run/web/')
def oxygenweb():
    website = request.args.get('website')
    r = requests.get(website)
    oxygen = Oxygen("\n" + r.text + "\n")
    result = oxygen.lexer()
    if result["error"] != False:
        return "Lexer: " + result["error"]
    else:
        return oxygen.parse(result["tokens"])

@app.route('/lib/add/<name>/')
def oxygenaddlib(name):
    website = request.args.get('website')
    thelibs.append({
        "name": name,
        "src": website
    })
    return jsonify(thelibs)

@app.route('/lib/check')
def checklibs():
    return jsonify(thelibs)

@app.route('/lib/getpackageurl')
def getpackageurl():
    pass
